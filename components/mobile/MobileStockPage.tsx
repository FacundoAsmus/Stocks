...
// A standalone collapsing search overlay — used only during the back animation.
// It starts fully expanded and immediately begins collapsing to the bottom-right circle.
function CollapsingSearchOverlay({ onDone }: { onDone: () => void }) {
  const [expanded, setExpanded] = useState(true);

  useEffect(() => {
    // Start collapsing immediately on mount
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: "width 300ms cubic-bezier(0.4,0,0.2,1), height 300ms cubic-bezier(0.4,0,0.2,1), border-radius 300ms cubic-bezier(0.4,0,0.2,1), bottom 300ms cubic-bezier(0.4,0,0.2,1), right 300ms cubic-bezier(0.4,0,0.2,1)",
      }}
    />
  );
}
...
export function MobileStockPage({ stock, currentPrice, sentiment, metrics }: MobileStockPageProps) {
  ...
  function handleBack() {
    const el = pageRef.current;
    if (fromSearch) {
      // Play the search closing animation, then navigate to where the user came from.
      // The destination page's loading screen will be revealed underneath as the circle shrinks.
      setPlayingCloseAnim(true);
      // Navigate immediately so the destination page (and its loading state) starts
      // rendering behind the collapsing overlay
      router.push(searchOrigin as "/" | "/watchlist");
      // CollapsingSearchOverlay calls onDone after 320ms — nothing more needed here
    } else if (el) {
  ...
  return (
    <>
      {/* Collapsing search overlay — only shown when pressing back from a search-opened page */}
      {playingCloseAnim && (
