...
// The individual stock page's own chart (PriceChart) sits in a fluid-width
// column at a fixed 320px height, so its own on-screen aspect ratio (width
...
function GraphFrame({ title, ready, tall, children }: { title: string; ready: boolean; children: React.ReactNode; isLightMode?: boolean; tall?: boolean }) {
  ...
  // Width is left exactly as the surrounding layout already sizes it (chat
  // bubble / panel width is untouched) — only the height adapts to keep the
  // chart's proportions consistent as that width changes across screen sizes.
  useEffect(() => {
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={points} margin={{ top: hasAnnotations ? 14 : 4, right: 4, bottom: 0, left: 4 }}>
            ...
            {annotations?.filter((a): a is MarkAnnotation => a.type === "mark").map((a, i) => {
              ...
              // sits in the top half of the chart, drop the label below it,
              // and if it sits in the bottom half, lift the label above it.
              const yProp = (a.price - priceDomainMin) / priceSpan;
              const ySide: "above" | "below" = yProp >= 0.5 ? "below" : "above";
